...
FIG = "/home/jovyan/work/project/paper/figures/internet_pathway_2023.png"
...
def main():
    ...
    for x in (x1, x2, x3, x4):
        demark(ax, x, "1" if x == x1 else "2" if x == x2 else "3" if x == x3 else "4", 0.98)

    # pathway segments, content (left) -> user (right)
    ...
    # data-flow direction: content -> user
    ym = y0 + h / 2
    flow(ax, x1 - 0.012, x1 + 0.014, ym)
    flow(ax, x2 - 0.012, x2 + 0.014, ym)
    flow(ax, x3 - 0.012, x3 + 0.014, ym)
    flow(ax, x4 - 0.012, x4 + 0.014, ym)
    ax.text(x1 / 2, y0 - 0.045, "data flow", ha="center", va="center", fontsize=8.5,
            color=DGREY, style="italic")

    # annotations: where congestion/latency sits in the 2023 model
    ax.annotate("persistent congestion:\nlegacy copper last mile",
                xy=(x3 + (x4 - x3) * 0.55, y0), xytext=(0.63, 0.115),
                fontsize=8.5, color=RED, fontweight="bold", ha="center",
                arrowprops=dict(arrowstyle="->", color=RED, lw=1.2))
    ax.annotate("under-provisioned\ninterconnection",
                xy=(x2, y0), xytext=(0.34, 0.125),
                fontsize=8.5, color=RED, fontweight="bold", ha="center",
                arrowprops=dict(arrowstyle="->", color=RED, lw=1.2))
    ax.annotate("latency now set by the\nlast mile + distance to the\nnearest edge cache — not\ntransit across the core",
                xy=(x1 - 0.02, y0 + h), xytext=(0.085, 0.995),
                fontsize=8.5, color=BLUE, ha="center", va="top",
                arrowprops=dict(arrowstyle="->", color=BLUE, lw=1.2))

    ax.set_title("2023+ Internet pathway model: content at the edge, user at the periphery",
